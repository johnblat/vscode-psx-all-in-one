 ...
 */
/* ---------------------------------------------------------------
 *  オーバーレイによって子プロセス（.BINファイル）を読み込み、
 *  関数として呼び出す。サウンドは鳴らしたままにする。
 *===============================================================
 */
#include <sys/types.h>
#include <sys/file.h>
...
/* 子プロセスのロードアドレス。ADDRESS.S で定義されている */
...
/* 一旦、子プロセスと同じ場所に置く。SPUに転送後は壊される。 */
#define VB_ADDR 0x80070000 

#define SEQ_SIZE (12738/2048+1)*2048		/* 親プロセス側に常駐 */
#define VH_SIZE (3104/2048+1)*2048		/* 親プロセス側に常駐 */
...
main()
{
  ...
  while(1){
	...
	Puts(MENU_X,y,dm[j].s);
      ...
      if (pad & PAD_A_START){
	file_read(dm[i].fn,LoadAddress);/* 子プロセスロード */
	dm[i].func();		/* 子プロセス実行 */
	break;
      }
      if (pad & PAD_A_UP)
...
void *address;
{
  ...
  for (i = 0; i < 10; i++) {	/* リトライは 10 回 */
    ...
    cnt = _read1(fp.size,address,mode); /* リード */
