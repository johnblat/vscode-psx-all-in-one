...
#define REP_NUM1 10	/* 始めのリピートするまでのタイミング */
#define REP_NUM2 3	/* リピートのタイミング */

/*
 * Primitive Buffer
 */
#define OTSIZE		8		/* OT の大きさ */
typedef struct {
	DRAWENV		draw;		/* 描画環境 */
	DISPENV		disp;		/* 表示環境 */
	u_long		ot[OTSIZE];	/* OT */
...
main()
{
	...
	DB	*cdb;		/* ダブルバッファのうちの現在のポインタ */
	int	depth=0;	/* 階層 */
	int	wait=0;		/* work */
	int	newdepth;	/* work */
	FLAGS	fl={JAPANESE,0,0,0};	/* flags */

	static int		menu_max;	/* メニューアイテムの数 */
	static int		execno = 0;
...
static int pad_read(MENU *menu,int depth,DISP_MENU *dm,int *execno,FLAGS *fl)
{
	...
	static int	f_rep = 0;  /* リピートのためのフラグ */
	static int	pcnt  = 0;  /* ボタンが押されていた数 */
