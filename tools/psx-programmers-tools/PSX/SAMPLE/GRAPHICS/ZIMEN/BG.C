...
MATRIX *ls;
{
	...
	offnw  = -outs.vx/bp->cw-field_nw; /* X軸 開始セル番号 */
	offnh  = -outs.vy/bp->ch-field_nh; /* Y軸 開始セル番号 */
	startx = offnw*bp->cw;   /* X軸開始セル位置（マップ座標）*/
	starty = offnh*bp->ch;     /* Y軸開始セル位置（マップ座標）*/
	...
	/* 左上の点を原点に持ってくる */
	ls->t[0]+=outs.vx;		/* X軸開始セル位置（スクリーン座標）*/
	ls->t[1]+=outs.vy;		/* Y軸開始セル位置（スクリーン座標）*/
	ls->t[2]+=outs.vz;		/* Z軸開始セル位置（スクリーン座標）*/
	...
	startx = starty = 0;
	/* LS-MATRIXにstartx,startyを反映させたので
	    startx,startyは0にリセット */

	v2.vy = starty;
  
	for(i=0;i<field_nh+field_nh1;i++) {
		v0.vy = v1.vy = v2.vy;	/* 次の行のyのセッテイング */
		v2.vy = v3.vy = v0.vy+bp->ch;
		v1.vx = startx;		/* 次の行のxのセッティング */
		...
		for(j=0;j<field_nw+field_nw1;j++) {
			...
			v0.vx = v2.vx = v1.vx; /* 次の列のxのセッティング */
