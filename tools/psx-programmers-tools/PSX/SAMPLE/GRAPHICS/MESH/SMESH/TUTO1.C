...
#define MESH_LEN	150			/* number of vertex */
...
main()
{
	...
	for(i=0;i<PRIM_LEN;i++){
		if(i%2==0){
			v0.vx= p_G3[i+1].vx - p_G3[i].vx;
			v0.vy= p_G3[i+1].vy - p_G3[i].vy;
			v0.vz= p_G3[i+1].vz - p_G3[i].vz;
			v1.vx= p_G3[i+2].vx - p_G3[i].vx;
			v1.vy= p_G3[i+2].vy - p_G3[i].vy;
			v1.vz= p_G3[i+2].vz - p_G3[i].vz;
		}else{
			v0.vx= p_G3[i].vx - p_G3[i+1].vx;
			v0.vy= p_G3[i].vy - p_G3[i+1].vy;
			v0.vz= p_G3[i].vz - p_G3[i+1].vz;
			v1.vx= p_G3[i+2].vx - p_G3[i+1].vx;
			v1.vy= p_G3[i+2].vy - p_G3[i+1].vy;
			v1.vz= p_G3[i+2].vz - p_G3[i+1].vz;
		}	
	...
	while (pad_read() == 0) {
		...
		RotMeshPrimS_G3(&msh_G3,cdb->surf_G3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_F3(&msh_F3,cdb->surf_F3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_FT3(&msh_FT3,cdb->surf_FT3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_GT3(&msh_GT3,cdb->surf_GT3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_GC3(&msh_GC3,cdb->surf_GC3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_FC3(&msh_FC3,cdb->surf_FC3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_FCT3(&msh_FCT3,cdb->surf_FCT3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_GCT3(&msh_GCT3,cdb->surf_GCT3,cdb->ot,OTLEN,dpq,backc);

		RotMeshPrimS_T3(&msh_T3,cdb->surf_T3,cdb->ot,OTLEN,dpq,backc);
...
DB	*db;
{
	...
	for(i=0;i<PRIM_LEN;i++){
		...
		SetPolyGT3(&db->surf_GT3[i]);	/* set POLY_GT3 primitve ID */
