...
/* ‚‘¬‰»‚Ì‚½‚ß‚Ì’è‹`  */
#define FASTVBLNK	/* VBLNK ‚ğ’Z‚­‚·‚é */
#define SMALLDRAWAREA	/* •`‰æ‚ğŒ¸‚ç‚· */
#define OTENV		/* PutDrawEnv ‚Æ DrawOTag ‚ğ“¯‚És‚¤ */

/* Š¿š‚ğƒvƒŠƒ“ƒg‚·‚é‚½‚ß‚Ì’è‹` */
#define KANJI

/*#define DEBUG */
/* ƒvƒŠƒ~ƒeƒBƒuŠÖ˜A‚Ìƒoƒbƒtƒ@ */
#define OTSIZE		1		/* ƒI[ƒ_ƒŠƒ“ƒOƒe[ƒuƒ‹‚Ì” */
#define MAXOBJ		4000		/* ƒXƒvƒ‰ƒCƒgiƒ{[ƒ‹j”‚ÌãŒÀ */
typedef struct {
	DRAWENV		draw;		/* •`‰æŠÂ‹« */
	DISPENV		disp;		/* •\¦ŠÂ‹« */
	u_long		ot[OTSIZE];	/* ƒI[ƒ_ƒŠƒ“ƒOƒe[ƒuƒ‹ */
	SPRT_16		sprt[MAXOBJ];	/* 16x16ŒÅ’èƒTƒCƒY‚ÌƒXƒvƒ‰ƒCƒg */
} DB;

/* ƒXƒvƒ‰ƒCƒg‚Ì“®‚«‚ÉŠÖ‚·‚éƒoƒbƒtƒ@ */
typedef struct {
	u_short x, y;			/* Œ»İ‚ÌˆÊ’u */
	u_short dx, dy;			/* ‘¬“x */
} POS;

/* •\¦—Ìˆæ */
#define	FRAME_X		640		/* •\¦—ÌˆæƒTƒCƒY(320x240)*/
#define	FRAME_Y		480
#define WALL_X		(FRAME_X-16)	/* ƒXƒvƒ‰ƒCƒg‚Ì‰Â“®—ÌˆæƒTƒCƒY */
#define WALL_Y		(FRAME_Y-16)

/* ƒvƒŠƒ~ƒeƒBƒuƒoƒbƒtƒ@‚Ì‰Šúİ’è */
static void init_prim(DB *db);	

/* ƒRƒ“ƒgƒ[ƒ‰‚Ì‰ğÍ */
static int  pad_read(int n);	

/* V-Sync‚ÌƒR[ƒ‹ƒoƒbƒNƒ‹[ƒ`ƒ“ */
static void  cbvsync(void);	

/* ƒ{[ƒ‹‚ÌƒXƒ^[ƒgˆÊ’u‚ÆˆÚ“®‹——£‚Ìİ’è */
...
main()
{
	...
	/* ƒ_ƒuƒ‹ƒoƒbƒtƒ@‚Ì‚½‚ß‚Q‚Â—pˆÓ‚·‚é */
	DB	db[2];		
	
	/* Œ»İ‚Ìƒ_ƒuƒ‹ƒoƒbƒtƒ@ƒoƒbƒtƒ@‚ÌƒAƒhƒŒƒX */
	DB	*cdb;		

	/* •\¦‚·‚éƒXƒvƒ‰ƒCƒg‚Ì”iÅ‰‚Í‚P‚Â‚©‚çj*/
	int	nobj = 1;	
	...
	/* •`‰æE•\¦ŠÂ‹«‚ÌƒŠƒZƒbƒg(0:cold,1:warm)  */
	ResetGraph(0);		
	
	/* ƒfƒoƒbƒOƒ‚[ƒh‚Ìİ’è(0:off,1:monitor,2:dump)*/
	SetGraphDebug(0);	
	
	/* V-sync‚ÌƒR[ƒ‹ƒoƒbƒNŠÖ”‚Ìİ’è*/
...
*/
	...
	/* ƒvƒŠƒ~ƒeƒBƒuƒoƒbƒtƒ@‚Ì‰Šúİ’è(db[0])*/
	init_prim(&db[0]);	
	
	/* ƒvƒŠƒ~ƒeƒBƒuƒoƒbƒtƒ@‚Ì‰Šúİ’è(db[1])*/
	init_prim(&db[1]);	
	
	/* ƒ{[ƒ‹‚Ì“®‚«‚ÉŠÖ‚·‚é‰Šúİ’è */
	init_point(pos);	

	/* ƒfƒBƒXƒvƒŒƒC‚Ö‚Ì•\¦ŠJn */
	SetDispMask(1);		/* ‚OF•s‰Â  ‚PF‰Â */

	/* ƒƒCƒ“ƒ‹[ƒv*/
	while ((nobj = pad_read(nobj)) > 0) {
		/* ƒ_ƒuƒ‹ƒoƒbƒtƒ@ƒ|ƒCƒ“ƒ^‚ÌØ‚è‘Ö‚¦ */
		cdb  = (cdb==db)? db+1: db;	
#ifdef DEBUG
		/* dump DB environment */
...
static void init_prim(DB *db)
{
	u_short	clut[32];		/* ƒeƒNƒXƒ`ƒƒ CLUT */
	SPRT_16	*sp;			/* work */
	int	i;			/* work */
	...
	for (sp = db->sprt, i = 0; i < MAXOBJ; i++, sp++) {
		/* 16x16ƒXƒvƒ‰ƒCƒgƒvƒŠƒ~ƒeƒBƒu‚Ì‰Šú‰» */
		SetSprt16(sp);		
		
		/* ”¼“§–¾‘®« ƒ ON */
		SetSemiTrans(sp, 1);	
		
		/* ƒVƒF[ƒfƒBƒ“ƒO‚ğs‚í‚È‚¢ */
		SetShadeTex(sp, 1);	
