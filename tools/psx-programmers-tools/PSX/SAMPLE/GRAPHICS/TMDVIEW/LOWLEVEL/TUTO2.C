 ...
 *
 * "tuto2.c" ******** simple GsDOBJ4 Viewing routine using GsTMDfast only for
...
#define OBJECTMAX 100		/* defines the maximum number of logical objects
a 3D model can be divided into :;３Dのモデルは論理的なオブジェクトに 分けられるこの最大数
				 * を定義する */

#define PACKETMAX2 (PACKETMAX*24)	/* size of PACKETMAX (byte) 
				packet size may be 24 byte(6 word) */

#define TEX_ADDR   0x80120000	/* テクスチャデータ（TIMフォーマット）
				 * がおかれるアドレス */
...
#define MODEL_ADDR 0x80100000	/* モデリングデータ（TMDフォーマット）
				 * がおかれるアドレス */
...
u_long  Objnum;			/* モデリングデータのオブジェクトの数を 保持する */
...
main()
{
	...
	while (1) {
		...
		GsClearOt(0, 0, &Wot[outbuf_idx]);	/* オーダリングテーブルをク
							 * リアする */
...
init_all()
{
	...
	Wot[0].length = OT_LENGTH;	/* オーダリングテーブルハンドラに解像度
					 * 設定 */
	Wot[0].offset = 0x7ff;
...
model_init()
{
	...
	dop = (u_long *) MODEL_ADDR;	/* モデリングデータが格納されているアド
					 * レス */
	dop++;			/* hedder skip */
...
extern PACKET *FastG4LTrr(); /* see g4trr.c */
extern PACKET *FastTG4LTrr(); /* see tg4trr.c */
	...
	int     shift;
{
	...
	while (primn) {
		...
		switch (code) {
			case GPU_COM_G4:
				GsOUT_PACKET_P = (PACKET *) FastG4LTrr
				...
				break;
			case GPU_COM_TG4:	
				GsOUT_PACKET_P = (PACKET *) FastTG4LTrr
