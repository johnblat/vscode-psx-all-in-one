...
#define TEX_ADDR   0x80010000	/* テクスチャデータ（TIMフォーマット）がおかれるアドレス */
...
GsOT    Wot1[2];		/* オーダリングテーブルハンドラダブルバッファのため２つ必要 */

GsOT    Wot2[2];		/* オーダリングテーブルハンドラダブルバッファのため２つ必要 */
...
main()
{
	...
	while (1) {
		...
		for (i = 0, op = object; i < Objnum; i++) {	/* オブジェクト１計算 */
		...
		for (i = 0, op = &object[Objnum]; i < Objnum; i++) {	/* オブジェクト２計算 */
			/* ワールド／ローカルマトリックスを計算する */
			GsGetLw(op->coord2, &tmpls);
			/* ライトマトリックスをGTEにセットする */
			GsSetLightMatrix(&tmpls);
