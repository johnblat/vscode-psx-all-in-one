 ...
 *
 *	"tuto5.c" ******** using GsSortObject5J() (minimize the program size)
 *
...
main()
{
  ...
  while(1)
      ...
      for(i=0,op=object;i<Objnum;i++)
	{
	  ...
	  GsSortObject5J(op,&Wot[outbuf_idx],14-OT_LENGTH,getScratchAddr(0));
	  op++;
	}
...
init_all()			/* 初期化ルーチン群 */
{
  ...
  jt_init();			/* GsFCall5 jump table init */
}
...
extern _GsFCALL GsFCALL5;	/* jump table of GsSortObject5J() */

/* 関数テーブルに 適切な関数を登録する */

/* 以下の例では フラット３角形 光源計算有り のみ 登録している */

jt_init()
{
  PACKET *GsPrstF3L(); /* dmyGsPrstF3L() */
  PACKET *GsPrstTF3L(); /* dmyGsPrstTF3L() */
  
  GsFCALL5.f3[GsDivMODE_NDIV][GsLMODE_NORMAL]  = GsPrstF3L;/* dmyGsPrstF3L */
  GsFCALL5.tf3[GsDivMODE_NDIV][GsLMODE_NORMAL] = GsPrstTF3L;/* dmyGsPrstTF3L */
}
