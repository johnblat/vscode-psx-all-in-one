...
/* 頂点差分データ (vdf file) の置かれるアドレス */
#define VDF_ADDR ((u_long *)0x80108000)
/* モデルひとつの頂点数の最大値 */
#define VTXMAX 24
/* 元頂点データの退避場所 */
u_long orgvtx[VTXMAX*sizeof(SVECTOR)];
...
#endif /* 0 */
int model_init(void);
static void model_mime_init(void);
...
static void model_mime_init(void)
{
    ...
    for (i=0; i<DISPNUM; i++){
	init_mime_data(i, TMD_ADDR, VDF_ADDR, NULL, orgvtx, NULL);
    }
	...
	mimepr[ARR_INTR][i]=0;
	for (j=0; j<MIMENUM; j++){
	    if (j==i) mimepr[ARR_MIME[j]][i]=4096;
	    else  mimepr[ARR_MIME[j]][i]=0;
	}
    }
...
#define RT 10
    ...
    /* MIMe */
    for (i=0; i<MIMENUM; i++){
	mimepr[ARR_INTR][i]=ctlfc[i].out;	/* set MIMe parameter */
    }
    ...
    intrm->reset_func();
    intrm->intrpol_func(&mimepr[ARR_INTR][0]);
