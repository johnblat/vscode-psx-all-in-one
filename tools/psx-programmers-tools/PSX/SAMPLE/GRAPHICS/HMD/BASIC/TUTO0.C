 ...
 *		All rights Reserved
 *
 *	This program does not support Animation and MIMe primitives.
 *
 */
...
int obj_interactive()
{
	...
	if (padd != opadd) {
		if (padd & PADR2)	num++;
		if (padd & PADL2)	num--;
		if (padd & PADstart)	num = 0;
		if (padd & PADselect)	num = bnum-3;
		limitRange(num, 0, bnum-3);
	}
	...
	if (padd & PADL1)	DModel[num].matrix.t[2] += 50;
	if (padd & PADR1)	DModel[num].matrix.t[2] -= 50;
...
void model_init(void)
{
	...
	dop = MODEL_ADDR;	/* モデリングデータが格納されているアドレス */
