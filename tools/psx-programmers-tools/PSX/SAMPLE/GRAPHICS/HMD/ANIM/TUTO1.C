...
/*
 *	"tuto0.c" HMD ANIMATION viewer (Realtime Motion Switch #1)
 *
 *		Copyright (C) 1997  Sony Computer Entertainment
 *		All rights Reserved
 *
 *	This program works with data/hmd/scei/anim/tri3/tri.hmd
 *	or data/hmd/scei/anim/tri4/tri.hmd
 *
 */
...
int obj_interactive()
{
	...
	if (padd != opadd) {
	        if (padd & PADR2)	switch_anim0();
	        if (padd & PADL2)	switch_anim1();
		if (padd & PADstart)	switch_anim2();
		if (padd & PADselect)	switch_anim3();
	}
