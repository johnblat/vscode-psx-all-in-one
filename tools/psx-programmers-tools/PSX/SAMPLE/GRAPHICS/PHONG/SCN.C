 ...
 * 
 *	このプログラムは１個の三角形をフォンシェーディングする
 *	関数phong_triを含みます。phong_triは「三角形の色」、
 *	「頂点の法線ベクトル」、「頂点のスクリーン座標」が与えられた
 *	ときフォンシェーディングをして描画します。
 *	ただし、phong_triは直接ビデオメモリに描画しますので、
 *	他のポリゴンとZsortしたいときは、いったん非表示領域に
 *	描画しテクスチャマップしなおす必要があります。
 *
 *		1995,3,29	by Oka
