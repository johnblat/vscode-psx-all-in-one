...
init_all()			/* 初期化ルーチン群 */
{
  ...
  GsInitGraph(640,480,2,0,0);	/* （インターレースモード） */
  GsDefDispBuff(0,0,0,0);	/* ダブルバッファ指定 */

/*  GsInitGraph(640,240,0,0,0);	/* （ノンインターレースモード）*/
/*  GsDefDispBuff(0,0,0,240);	/* ダブルバッファ指定 */

  GsInit3D();			/* ３Dシステム初期化 */
  
  Wot[0].length=OT_LENGTH;	/* OTハンドラに解像度設定 */
  Wot[0].org=zsorttable[0];	/* OTハンドラにOTの実体設定 */
  				/* ダブルバッファ*/
