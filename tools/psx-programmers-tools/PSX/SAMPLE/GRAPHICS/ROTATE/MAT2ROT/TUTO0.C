...
#define OT_LENGTH  1	/* オーダリングテーブルの解像度 */
...
GsOT_TAG	zsorttable[2][1<<OT_LENGTH]; /* OT実体 */
...
GsCOORDINATE2   DWorld;	/* オリジナル座標系 */
GsCOORDINATE2   Dnew;	/* 再構成座標系 */
...
InvRotMatrixZYX(MATRIX *rotm, SVECTOR *vector)
{
    ...
    /* 平行移動をしない回転だけのマトリクスにする */
    m= *rotm;
    m.t[0]=m.t[1]=m.t[2]=0;

    /* 基準点 A (={0,0,1})を回転させてみる */
