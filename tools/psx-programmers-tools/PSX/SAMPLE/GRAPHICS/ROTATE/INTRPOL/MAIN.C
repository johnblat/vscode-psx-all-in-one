...
#define PNUM	6		/* 面の数 */
#define CUBESIZE	80	/* 辺のながさ */
#define NINTP	100		/* 内挿の解像度 */
#define VNUM 8			/* 頂点の数 */

#define Printf(x) FntPrint x
int kstream[3];

#define CUBENUM 5		/* 立方体の数 */
...
typedef struct {
    POLY_F4 surf[PNUM][2];	/* Cube 表面の正方形x6個 */
    GsCOORDINATE2 coord2;	/* Cube の座標系 */
    SVECTOR sv;			/* Cube の回転角(使わないこともある) */
} PBUF;				/* struct of cube */
...
init_all()
{
    ...
    Wot[0].length=OT_LENGTH;	/* Wot の解像度設定 */
    Wot[0].org=zsorttable[0];	/* Wot にオーダリングテーブルの実体設定 */
