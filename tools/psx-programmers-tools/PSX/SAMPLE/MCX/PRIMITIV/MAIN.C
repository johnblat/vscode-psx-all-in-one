...
static unsigned char *enable[]={"disable", "enable "};
...
static int paramExecApr(int, int);
static int paramCurrCtrl(int, int);
static int paramFlushAcs(int, int);
...
static void dispMenu()
{
	...
	FntPrint("  %s3: flush access    ~c888 %s 9: set led         ~c888 %s15: set memory    ~c888\n",cmdcol[2],cmdcol[8],cmdcol[14]);
	...
	FntPrint("  %s6: get aplli       ~c888 %s12: read device     ~c888 %s18: get user ifs  ~c888\n",cmdcol[5],cmdcol[11],cmdcol[17]);
	FntPrint("\n");
}
...
static int manageParam(int mode, int button)
{
	...
	case 2:
		return paramFlushAcs(mode, button);
	case 3:
		return paramExecFlag(mode, button);
	case 4:
		return paramExecApr(mode, button);
	case 6:
...
static int paramAllInfo(int mode)
{
	...
	FntPrint("    PDA appli flush access :%s \n",
		state[16] ? "SUPERIOR":"INFERIOR");
...
/*#paramGetApr --- */
static int paramGetApr(int mode)
{
	static long aprno;

	if(mode==2)
	{
		return McxGetApr(PortNo1, &aprno);
	}
...
static int paramHideTrans(int mode)
{
	...
	if(mode==2)
	{
		return McxHideTrans(PortNo1, TRUE);
	}
	dispNoData();
...
/*#paramExecApr --- */
static int paramExecApr(int mode, int button)
{
	...
	case 0:
		...
		break;
	case 1:
		FntPrint("    triang: inc appri no.     squar: dec appli no. \n\n");
		...
		break;
	case 2:
		return McxExecApr(PortNo1, data[0], 
...
/*#paramFlushAcs --- */
static int paramFlushAcs(int mode, int button)
{
	static int superior;
	...
	case 0:
		...
		break;
	case 1:
		FntPrint("    circle: flush write superior to spi \n");
		FntPrint("    cross:  spi superior to flush write \n\n");

		FntPrint("    PDA appli flush access : ~c558%s~c888 \n",
			superior ? "SUPERIOR":"INFERIOR");
		break;
	case 2:
		return McxFlushAcs(PortNo1, superior);
	case 3:
		dispNoData();
		break;
	}
