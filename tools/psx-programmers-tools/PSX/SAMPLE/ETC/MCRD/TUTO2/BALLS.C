...
void _make_balls_data( void )
{
	...
	tpage = LoadTPage(ball16x16, 0, 0, 960, 0, 16, 16);
	for (i = 0; i < 32; i++) {
		clut[i] = LoadClut(ballcolor[i], 768, 480+i );
	}
	DrawSync(0);
	...
	for( i=0; i<BALLMAX; i++ ) {
		...
		vx[i] = (rand()%2 ? 1:-1)*(rand()%5+1);
		vy[i] = (rand()%2 ? 1:-1)*(rand()%5+1);
	}

}

void _draw_balls_data( int side, long* ot, int balls )
{
	int	i;
	SPRT_16* sprt;
