...
#if IS_RGB24==1
#define PPW 3/2			/* 1 ショートワードに何ピクセルあるか  */
#define DCT_MODE    3		/* 24bit モードでデコード */
#else
#define PPW 1			/* 1 ショートワードに何ピクセルあるか */
...
typedef struct {
    ...
    RECT    slice;		/* 1 回の DecDCTout で取り出す領域 */
    int isdone;			/* 1フレーム分のデータができたか */
} DECENV;
...
static int  Rewind_Switch;	/* 終了フラグ 所定のフレームまで再生すると
				   1になる */
